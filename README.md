...
| 11.1 Chat Module                   | ✅ COMPLETE    | Chat sessions, persisted history, ownership/IDOR checks, idempotent retries, deletion and shared Worker AI provider; local stage11: 222 tests passed. Manual acceptance/security QA deferred |
| 11.2 AI Context & Streaming        | ✅ COMPLETE    | Context, memory, summary/cache and provider SSE → Worker → Redis → API SSE path implemented and covered by automated checks. Real-provider recovery/disconnect/browser QA deferred           |
| 12.1 Core Application              | ✅ COMPLETE    | Shared UI Kit/tokens, protected Web auth, refresh/logout, real Dashboard and layouts; local Web/Admin tests passed. Responsive/accessibility/browser QA deferred                             |
| 12.2 User Features                 | ✅ COMPLETE    | Projects, ZIP upload/versioning, analysis status, reports/exports/compare, chat UI and settings connected to API. Full real-data E2E/manual QA deferred                                      |
| 13.1 Admin Core                    | ✅ COMPLETE    | Admin RBAC, dedicated users/projects API, server-side search/pagination, details and role/status mutation implemented and tested. Manual IDOR/E2E/browser QA deferred                        |
| 13.2 Administration                | ✅ COMPLETE    | Queues, logs, AI usage, statistics, filters and safe job operations implemented through backend services and tested. Operational/manual QA deferred                                          |
| 14.1 OpenAPI & SDK                 | ✅ COMPLETE    | Canonical OpenAPI generation/validation, drift checks, typed streaming contract and SDK generation are implemented and tested. Extended manual contract QA deferred                          |
| 14.2 Frontend Integration          | ✅ COMPLETE    | Web/Admin share SDK client, auth/refresh, query layers, typed streaming and normalized API errors. Real-API/cache/browser QA deferred                                                        |
...
проектов. Administration дополнительно фильтрует логи по event/requestId/userId,
показывает event в таблице и поддерживает AI usage по user/project и периоды
статистики 24h/7d/30d. Этапы 11–14 считаются завершёнными на уровне реализации
и автоматических проверок; security/IDOR, real-API E2E и browser/manual QA
перенесены в отдельный последующий QA-проход.

```txt
Текущий результат сверки: этапы 11–14 готовы по реализации и автоматическим CI-
проверкам. В совокупности этапы 1–19 считаются готовыми на уровне текущего
implementation baseline. Ручные browser/security/real-E2E проверки сознательно
перенесены и будут выполнены отдельным QA-проходом. Подробная сверка: [`docs/implementation/stages-11-14-completion-audit.md`](docs/implementation/stages-11-14-completion-audit.md).
```
...
через `/docs-json`, health endpoint возвращает HTTP 200.
...
HTTP upload обрабатывается через временный файл и передаётся в MinIO потоком;
после завершения временный файл удаляется. Файлы `.git`, `node_modules`, `dist`,
`build` и `.env` запрещены внутри архива.
