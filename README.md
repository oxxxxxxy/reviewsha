...
При `AI_MERGE_FILES=true` анализ выполняется в два содержательных запроса: сначала модель получает дерево проекта и первые 100 символов каждого файла и выбирает до трёх наиболее важных файлов; затем получает их полное содержимое, дерево проекта и корневой `README.md` для объединённого ревью. Подготовительный запрос выбора не входит в счётчик ревью. После общего ревью отдельный файл можно отправить на дополнительный точечный review.
