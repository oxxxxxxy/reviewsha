...
export class ReportWorker extends BaseQueueWorker implements OnModuleInit, OnModuleDestroy {
  constructor(
    ...
    super(QUEUE_NAMES.report, logger);
  }
  ...
  async start(): Promise<void> {
    ...
    if (!this.queueService.isRedisAvailable()) {
      this.logger.warn('ReportWorker skipped: Redis unavailable', this.constructor.name);
      return;
    }
    ...
    this.bullWorker = new Worker(QUEUE_NAMES.report, (job) => this.processJob(job), { connection });
    this.queueService.registerWorker(this.bullWorker);
    this.logger.log('ReportWorker started', this.constructor.name);
  }
}
