...
export class CleanupWorker extends BaseQueueWorker implements OnModuleInit, OnModuleDestroy {
  constructor(
    ...
    super(QUEUE_NAMES.cleanup, logger);
  }
  ...
  async start(): Promise<void> {
    ...
    if (!this.queueService.isRedisAvailable()) {
      this.logger.warn('CleanupWorker skipped: Redis unavailable', this.constructor.name);
      return;
    }
    ...
    this.bullWorker = new Worker(QUEUE_NAMES.cleanup, (job) => this.processJob(job), {
      connection,
    });
    this.queueService.registerWorker(this.bullWorker);
    this.logger.log('CleanupWorker started', this.constructor.name);
  }
}
