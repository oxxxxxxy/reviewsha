...
export class UploadWorker extends BaseQueueWorker implements OnModuleInit, OnModuleDestroy {
  constructor(
    ...
    super(QUEUE_NAMES.upload, logger);
  }
  ...
  async start(): Promise<void> {
    ...
    if (!this.queueService.isRedisAvailable()) {
      this.logger.warn('UploadWorker skipped: Redis unavailable', this.constructor.name);
      return;
    }
    ...
    this.bullWorker = new Worker(QUEUE_NAMES.upload, (job) => this.processJob(job), { connection });
    this.queueService.registerWorker(this.bullWorker);
    this.logger.log('UploadWorker started', this.constructor.name);
  }
}
